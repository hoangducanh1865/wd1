...
def sinkhorn_torch(a, b, M, reg, numItermax=1000, stopThr=1e-9, verbose=False, log=False):
    ...
    # Initialize u and v
    u = torch.ones((batch_size, n), device=M.device) / n
    v = torch.ones((batch_size, m), device=M.device) / m
...
class OTWD1Trainer(RevDiffuGRPOTrainer):
    ...
    def get_embeddings(self, model, input_ids, attention_mask):
        ...
        last_indices = attention_mask.size(1) - attention_mask.flip(dims=[1]).to(dtype=torch.bool).int().argmax(dim=1) - 1
